...
# Runtime-generated bytecode is not source drift, while script edits are.
# Keep this fixture separate from the checkout so concurrent suites may compile
# their own modules without this test mutating source or an installed runtime.
BYTECODE_SOURCE="$TEST_ROOT/bytecode-source"
BYTECODE_TARGET="$TEST_ROOT/bytecode-target"
mkdir -p "$BYTECODE_SOURCE/skills/synthesis-bytecode-fixture"
printf '%s\n' '---' 'name: synthesis-bytecode-fixture' 'description: Fixture' '---' '# Fixture' \
    > "$BYTECODE_SOURCE/skills/synthesis-bytecode-fixture/SKILL.md"
printf '%s\n' 'value = 1' > "$BYTECODE_SOURCE/skills/synthesis-bytecode-fixture/fixture.py"
SYNTHESIS_CLAUDE_BIN= SYNTHESIS_CODEX_BIN= \
SYNTHESIS_SKILLS_HOME="$TEST_ROOT" \
SYNTHESIS_SKILLS_SOURCE_DIR="$BYTECODE_SOURCE" \
SYNTHESIS_SKILLS_TARGETS="$BYTECODE_TARGET" \
    "$DIRECT_COPY" install >/dev/null
python3 -m compileall -q "$BYTECODE_SOURCE" "$BYTECODE_TARGET"
SYNTHESIS_CLAUDE_BIN= SYNTHESIS_CODEX_BIN= \
SYNTHESIS_SKILLS_HOME="$TEST_ROOT" \
SYNTHESIS_SKILLS_SOURCE_DIR="$BYTECODE_SOURCE" \
SYNTHESIS_SKILLS_TARGETS="$BYTECODE_TARGET" \
    "$DIRECT_COPY" status >/dev/null || {
        echo "status treated generated bytecode as source drift" >&2
        exit 1
    }
printf '%s\n' 'value = 2' > "$BYTECODE_TARGET/synthesis-bytecode-fixture/fixture.py"
if SYNTHESIS_CLAUDE_BIN= SYNTHESIS_CODEX_BIN= \
   SYNTHESIS_SKILLS_HOME="$TEST_ROOT" \
   SYNTHESIS_SKILLS_SOURCE_DIR="$BYTECODE_SOURCE" \
   SYNTHESIS_SKILLS_TARGETS="$BYTECODE_TARGET" \
       "$DIRECT_COPY" status >/dev/null 2>&1; then
    echo "status accepted modified Python source" >&2
    exit 1
fi
