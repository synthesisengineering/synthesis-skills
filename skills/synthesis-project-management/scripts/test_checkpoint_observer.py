...
def test_missing_native_validator_fails_closed(observer: SimpleNamespace, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture) -> None:
    ...
    assert wire["continue"] is False and "remains FAIL" in wire["stopReason"]
    assert "decision" not in wire and "reason" not in wire
    assert report["status"] == "FAIL" and report["checkpoint_accepted"] is False
    assert_no_receipt(observer)
...
def test_native_reentrant_stop_terminates_with_blocked_verdict(observer: SimpleNamespace, capsys: pytest.CaptureFixture, client: str) -> None:
    ...
    # Neither first entry nor re-entry has an owner correction reservation.
    assert first_output["continue"] is False
    assert "remains UNKNOWN" in first_output["stopReason"] and "Preserve retained work" in first_output["stopReason"]
    assert "decision" not in first_output and "reason" not in first_output
    assert first_report["status"] == "UNKNOWN" and first_report["checkpoint_accepted"] is False
    payload["stop_hook_active"] = True
    assert state._emit_checkpoint_hook(verdict, issues, payload) == 0
    repeated = capsys.readouterr()
    terminal, report = native_output(repeated.out)
    assert report == first_report and terminal["continue"] is False
...
@pytest.mark.parametrize("client,active,event_name", [("codex", True, "Stop"), ("claude", "true", "Stop"),
    ("claude", True, "PreCompact"), ("codex", False, "PreToolUse"), ("claude", False, "PreToolUse")])
def test_terminal_control_is_shared_by_stop_clients_but_not_other_events(observer: SimpleNamespace, capsys: pytest.CaptureFixture, client: str, active: object, event_name: str) -> None:
    ...
    if event_name == "Stop":
        ...
        assert "Project checkpoint remains FAIL" in output.err
        assert "retained owner obligation" in output.err
    assert observer.board.read_bytes() == before
    assert_no_receipt(observer)
...
def test_real_hook_cli_uses_local_lease_and_actionable_failure(observer: SimpleNamespace) -> None:
    ...
    assert wire["continue"] is False and "remains UNKNOWN" in wire["stopReason"]
    assert "decision" not in wire and "reason" not in wire
...
def test_stop_output_conforms_to_native_codex_consumer_schema(observer: SimpleNamespace, capsys: pytest.CaptureFixture, verdict: str) -> None:
    ...
    assert set(wire) == ({"systemMessage"} if verdict in {"PASS", "NOT_APPLICABLE"}
                         else {"systemMessage", "continue", "stopReason"})
    if verdict not in {"PASS", "NOT_APPLICABLE"}:
        assert wire["continue"] is False and "bounded fixture evidence" in wire["stopReason"]
