...
- [ ] Read the returned source items and reconcile their current outcomes. `BLOCKED` or any unscanned source is an explicit coverage gap, never an empty pass; due items remain visible alongside gaps. `REVIEW` requires the decisions below. Only a complete, reconciled scan can support a clean closure; the helper's exit 0 grants no send authority. Workers report their own coverage in existing artifacts; the desk reports missing worker coverage without reading across workspace boundaries.
