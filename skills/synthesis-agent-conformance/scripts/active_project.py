...
def validate(
    ...
    *,
    ...
    if not owner_state or owner_state["status"] in TERMINAL_STATUSES:
        ...
        if str(worktree) not in owner_state["workspace"]:
            issues.append(
                f"owner session {owner} does not claim pointer worktree {worktree}"
            )
        try:
    ...
    if worktree.is_dir():
        ...
        if head.returncode != 0:
            ...
            if actual_head != payload.get("source_commit"):
                issues.append(
                    f"pointer source commit {payload.get('source_commit')} is not worktree HEAD {actual_head}"
                )
