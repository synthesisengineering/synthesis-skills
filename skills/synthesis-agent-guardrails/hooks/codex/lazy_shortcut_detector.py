#!/usr/bin/env python3
"""Codex Stop hook backed by the shared anti-shortcut catalog.

Twin of the Claude lazy_shortcut_detector: reads the Stop payload's
last_assistant_message from stdin, scans it with the shared detection core,
and appends detections to the Codex-local log. Log-only by design.
"""
...
LOG_FILE = Path.home() / ".codex" / "lazy-shortcut-log.jsonl"
...
def main(argv: list[str] | None = None) -> int:
    ...
    warning = (
        "The anti-shortcut detector found phrasing that can dismiss or postpone "
        f"work the user asked to complete: {phrases}. Rewrite from the stated constraints."
    )
