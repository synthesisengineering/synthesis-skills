...
def snapshot_current_source(source, target, env):
    """Bind pending source bytes to a Git fixture, never the developer's HEAD.

    Git enumerates tracked files plus nonignored additions. Untracked caches,
    checkout metadata and foreign worktrees are not copied; symlinks are never
    followed. This keeps real runtime source verification enabled before the
    source repository's changes have been committed.
    """
    names = sh(["git", "-C", str(source), "ls-files", "--cached", "--others",
                "--exclude-standard", "-z"], env=env).split("\0")
    target.mkdir()
    for name in sorted(set(filter(None, names))):
        relative = Path(name)
        if relative.is_absolute() or ".." in relative.parts or ".git" in relative.parts:
            raise AssertionError("unsafe fixture source path")
        original = source / relative
        # A removed tracked path stays removed. A nested Git checkout is a
        # directory entry, not a request to copy that checkout recursively.
        if not original.is_symlink() and not original.is_file():
            continue
...
def test_source_fixture_captures_pending_bytes_without_ignored_runtime_state(tmp_path):
    environment = {
        **os.environ, "GIT_CONFIG_GLOBAL": os.devnull, "GIT_CONFIG_NOSYSTEM": "1",
        "GIT_CONFIG_COUNT": "0", "GIT_AUTHOR_NAME": "Fixture",
    ...
    sh(["git", "-C", str(original), "init", "-q", "-b", "main"], env=environment)
    sh(["git", "-C", str(original), "add", "-A"], env=environment)
    sh(["git", "-c", "core.hooksPath=/dev/null", "-C", str(original), "commit", "-qm", "Fixture"], env=environment)
    (original / "helper.py").write_text("value = 2\n")
    (original / "new_helper.py").write_text("value = 3\n")
    (original / "__pycache__").mkdir()
    (original / "__pycache__" / "helper.pyc").write_bytes(b"generated bytecode")
    foreign = original / "foreign-worktree"
    foreign.mkdir()
    (foreign / "retained.txt").write_text("retained foreign fixture\n")
    sh(["git", "-C", str(foreign), "init", "-q"], env=environment)
    head_before = sh(["git", "-C", str(original), "rev-parse", "HEAD"], env=environment)
    status_before = sh(["git", "-C", str(original), "status", "--porcelain"], env=environment)

    snapshot = snapshot_current_source(original, tmp_path / "snapshot", environment)

    assert sh(["git", "-C", str(snapshot), "show", "HEAD:helper.py"], env=environment) == "value = 2\n"
    assert sh(["git", "-C", str(snapshot), "show", "HEAD:new_helper.py"], env=environment) == "value = 3\n"
    assert set(sh(["git", "-C", str(snapshot), "ls-files"], env=environment).splitlines()) == {
        ".gitignore", "helper.py", "new_helper.py",
    }
    assert not (snapshot / "__pycache__").exists()
    assert not (snapshot / "foreign-worktree").exists()
...
class Sandbox:
    ...
    def env_overrides(self):
        return {
            ...
            "SYNTHESIS_ONBOARD_SOURCE_DIR": str(getattr(self, "public_source", REPO_ROOT)),
    ...
    def isolate_public_source(self):
        self.public_source = snapshot_current_source(
            REPO_ROOT, self.root / "public-source", self.git_env,
        )
...
class EngineTests(unittest.TestCase):
    ...
    def test_full_init_converges_every_layer_and_reruns_cleanly(self):
        self.box.isolate_public_source()
