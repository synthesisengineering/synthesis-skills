...
def legacy_launcher_bytes(active_descriptor_path: Path) -> bytes:
    """Exact previously generated bytes; a marker alone never proves ownership."""
    return (
        "#!/bin/sh\n"
        + LAUNCHER_MARK
        + "\nset -eu\nexec python3 -B - "
...
def _activation_regular_bytes(path: Path) -> bytes | None:
    if path.is_symlink() or (path.exists() and not path.is_file()):
        raise ContractError("activation path is not a regular file: %s" % path)
    return path.read_bytes() if path.exists() else None


def _recover_activation(journal: Path, launcher_path: Path, active_path: Path) -> None:
    raw = _activation_regular_bytes(journal)
    if raw is None:
        return
    try:
...
def activate_cli(
    ...
    launcher_path = Path(launcher_path).absolute()
    ...
    with lock_path.open("a+b") as activation_lock:
        ...
        if current is not None:
            owned = current in {launcher, legacy_launcher_bytes(active_descriptor_path)}
